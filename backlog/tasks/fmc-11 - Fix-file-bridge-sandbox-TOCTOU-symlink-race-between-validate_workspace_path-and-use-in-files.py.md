...
Adversarial subagent review (per this project's convention for security-boundary fixes) found one real regression: write_file's rewritten I/O called a single os.write(fd, encoded) and discarded its return value, always reporting bytes_written=len(encoded) even though POSIX write() can short-write (e.g. ENOSPC, or non-local filesystems) -- the original resolved.write_bytes() used Python's buffered io layer which loops internally, so this was a reliability regression (not a TOCTOU/security hole) introduced by the fd-based rewrite. Fixed by looping os.write() over a memoryview until fully written, mirroring read_file's existing retry loop. Also added 2 boundary tests (read/write at exactly MAX_FILE_BYTES) the review flagged as an untested edge given the task's own note about the 10MB cap. Full suite now 317 passed (up from 315); ruff clean. Review also confirmed: all 3 call sites (including write_file's parent-mkdir step) close the race; FD lifecycle has no leaks; _root_for can never disagree with validate_workspace_path's root choice; no test is vacuous (pytest.raises paired with a direct filesystem assertion). Non-blocking notes from the review, left as-is (pre-existing/out of scope): O_NOFOLLOW does not defend against a pre-placed hard link (predates this fix, same limitation existed before); a raced IsADirectoryError is labeled ValidationError rather than PermissionDeniedError (taxonomy only, still fails closed either way).
<!-- SECTION:NOTES:END -->
