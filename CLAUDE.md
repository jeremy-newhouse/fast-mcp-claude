...
- **Launcher Codex engine env-scrub gap (FMC-19, verified live against codex-cli 0.145.0)**: `_scrubbed_env()` strips `MCP_API_KEY`/`CRM_*` from the env dict handed to `create_subprocess_exec`, which is complete protection for the Claude engine (`claude -p`'s Bash tool doesn't invoke a login shell). `codex exec`'s own shell tool, however, always runs commands via `/bin/zsh -lc` — a LOGIN shell — regardless of a `$SHELL` override or `-c shell_environment_policy.inherit=none` (both tested, neither helped); a login shell re-sources the operator's own `~/.zshrc`/`~/.zprofile`/`~/.zshenv`, which can reintroduce any secret exported there into a Codex task independent of the scrub. No known config fix exists. Operators must not export secrets in interactive shell startup files on any machine running the launcher with the Codex engine enabled.
