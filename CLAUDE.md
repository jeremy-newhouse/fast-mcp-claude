...
- `src/fast_mcp_claude/launcher.py` — `fast-mcp-claude-launcher` CLI; the *headless* exec leg (pm2-managed, not an MCP server). Long-polls the local server's inbox for tasks addressed to its launcher identity and spawns each as a `claude -p` subprocess in an allowlisted cwd under a tools ceiling, posting the result back via `reply`. Strict opt-in and zero-exec-by-default: the MCP server itself never spawns a subprocess, keeping the network surface free of an RCE primitive.
