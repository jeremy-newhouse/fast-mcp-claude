...
## Cursor
**Next issue: FMC-2** — queue order confirmed by the user on 2026-07-20 (selected
...
| --- | --- | --- | --- |
| 1 | FMC-2 | bug [HIGH] | who() leaks announce_token, defeating the ECA-71 identity guard |
| 2 | FMC-8 | bug [MED] | Verify/fix .mcp.json.example channel server-key mismatch vs channel.py's hardcoded SERVER_NAME |
| 3 | FMC-4 | bug [MED] | Close sandbox path-existence oracle + extend body-size caps to structured fields |
| 4 | FMC-5 | bug [MED] | Fix Notifier/long-poll correctness bugs in services/store.py (4 sub-bugs; riskiest change — touches core long-poll infra) |
| 5 | FMC-6 | bug [LOW] | Fix tool-pattern deviations from CLAUDE.md's documented contract |
