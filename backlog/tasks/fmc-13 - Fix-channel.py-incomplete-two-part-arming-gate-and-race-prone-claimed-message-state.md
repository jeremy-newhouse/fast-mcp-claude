...
Adversarial subagent review of the full branch diff (feature/FMC-13 vs dev) found no blocking issues in the 4 fixes -- confirmed correct: AC#1's presence if/elif mutual exclusion with the degraded branch, no legitimate-session hang risk in the initialized-wait loop (tee/inbox/presence all start concurrently via tg.start_soon), AC#2's ok-gated signal racing correctly against a mismatched-id late reply, AC#3's premise that record_reply's CAS genuinely refuses a double-finalize and that uuid4 message ids make false-positive matches a non-concern, and AC#4's try/except-wrapped push introducing no new interleaving risk versus the unchanged pre-existing position of the rt.inflight assignment. It DID catch one real, non-blocking gap: bounced_message_id was a single scalar, so only the MOST RECENT bounce got AC#3's distinguishable warning -- degrade_after (default 3) permits several consecutive bounces before the session disarms, so a late reply for an EARLIER bounce could arrive after a LATER bounce overwrote the scalar tracker and wrongly fall back to the generic unknown-id warning. Fixed by switching to a bounded deque (maxlen=64) of recently-bounced ids; added a dedicated regression test (test_reply_tool_after_bounce_distinguishes_earlier_bounce_too) confirmed via git stash to fail against the scalar version and pass post-fix. Also noted two informational, out-of-scope findings not requiring action: (a) the one-claimed-slot design's rt.inflight assignment happening before the push's await point is a pre-existing architectural property unchanged by this branch, not a regression; (b) a push failure clears rt.inflight (AC#4's actual promise) but does not itself bounce the abandoned message to the controller -- pre-existing behavior, not something AC#4's text commits to fixing.
<!-- SECTION:NOTES:END -->
