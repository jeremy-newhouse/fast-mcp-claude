...
- `src/fast_mcp_claude/session_hook.py` — `fast-mcp-claude-session-hook` CLI; a minimal, network-free `SessionStart`/`UserPromptSubmit`/`Stop` hook that writes the local JSON status file `session.py` reads each heartbeat. Deliberately least-privilege (no mesh bearer) so a compromised hook can't reach the mesh.
